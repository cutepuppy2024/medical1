...
-- 퀴즈 : 시퀀스 m_no는 시퀀스로 1-9999, 1씩 증가
-- m_yesterday, m_today, m_tomorrow, m_year 날짜컬럼을 가진 테이블 m_date
...
-- 회원들이 1년동안 로그인이 없으면, 법적으로 회원유지 동의
...
-- board 테이블 default 는 입력시 없을시 지정한 데이터 자동입력 됨
create table board(
bno number(4) primary key, -- 중복이 안됨, null 허용하지 않음. 기본키로 사용됨
id varchar2(30),
btitle varchar2(1000),
bcontent clob, -- varchar2(3000) clob -무제한, varchar2타입
...
-- pm/am : 상관없이 오전/오후 표기, hh24 : 24시간형태
...
-- 문자형 사칙연산(+,-,*,/) 안됨. 자리수표시, 쉼표표시, 날짜형태 출력 표시
-- 숫자형 사칙연산 가능 컬럼별 사칙연산 가능, 자리수표시(0001-> 안됨), 쉼표표시 안됨. 0001
-- 날짜형 +,- 연산기능가능, months-between 2개의 날짜 달 계산, 날짜유형을 지정해서 출력이 안됨. 
...
-- 퀴즈
-- 123,456,789 100,000
-- 123,456,789 100,000 = 값을출력하시오. 천단위 표시할것
-- 123,556,789

select 123456789+100000 from dual;
-- 123,456,789 의 쉼표를 제거 - replace('123,456,789',',','')
-- 타입을 number형으로 변경
-- 더하기를 함
-- 문자형 타입으로 원화, 쉼표 표시
-- total = '123,456,789'

select (123,456,789)+(100,000) from dual; -- 보통 이런 형식으로 데이터가 주어짐
...
-- 문자형태라도 날짜형태면 검색됨, +,-형태는 형변환해 주어야 함
...
-- manager_id,의 타입이 문자 -> 타입변경해주어야 데이터 변경이 가능
