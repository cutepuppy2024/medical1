...
sql = f'''select count(*) from ( select row_number() over(order by no) rnum,a.*
       ...
       '''
cursor.execute(sql)
all_count = cursor.fetchone()  # 튜플
limit = math.ceil((all_count[0])/perCount)  # 3.1
...
while True:
    ...
    sql =f'''select * from (select row_number() over(order by no) rnum,a.* from stu_score a where id like '%{search}%' )
