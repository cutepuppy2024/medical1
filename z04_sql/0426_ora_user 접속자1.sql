...
-- [ 무결성 제약조건 ]
-- foreign key는 다른 테이블에서 데이터 입력시 선언되어 있는 기존 테이블에 입력하려는 데이터가 있는지 확인
...
-- < 제약조건: primary key, not null, unique, check >
-- not null -  null값만 제외
create table emp02(
...
-- 컬럼의 내용추가 : departments의 데이터를 dept01 테이블로 가져옴
...
-- fk_cno가 삭제가 되도록 함.
-- fk키로 등록되어 있는 모든 데이터를 삭제시키는 것 : on delete cascade
